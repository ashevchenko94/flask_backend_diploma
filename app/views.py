...
def create_auto():
    ...
    if request.method == 'POST':
        ...
        # Добавляем авто в базу данных и сохраняем
...
def auto_detail(auto_id):
    ...
    if request.method == 'POST':
        ...
        if auto_transmission_check == 'option1':  #"Автоматическая КПП"
            auto.transmission = True
        elif auto_transmission_check == 'option2': # Механика
        ...
        db.session.commit() #сохраняем новую в информацию в поля таблицы БД
...
def auto_rent(auto_id): #функция арендовать/освободить авто
    ...
    if request.method == 'POST':
        ...
        if new_status == False: 
            ...
            auto.aurented = datetime.now() #записываем время начало аренды
            auto.date_end = None 
            ...
            free = 'свободен' 
            auto.date_end = datetime.now() #записываем время 
            age_seconds = (auto.date_end - auto.aurented).seconds  #время аренды в секундах
            age = divmod(age_seconds, 60) 
            total_price = age[0] * auto.autprice #стоимость аренды
            auto.atotal_time += age[0] #общее время аренды
            auto.total_price += total_price #общая стоимость аренды
            auto.rent_count += 1 #увеличиваем  общее количесвто бронирований авто
...
@app.route('/remove_auto/<int:auto_id>', methods=['POST'])
def remove_auto(auto_id): #функция удаления авто
    ...
    return render_template('remove_auto.html', **context)
